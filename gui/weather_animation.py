...
class WeatherAnimation:
    ...
    def _animate_frame(self):
        ...
        try:
            ...
            if self.frame_count % (self.fps * 5) == 0:
                print(f"[WeatherAnimation] Frame {self.frame_count}: {active_particles} active particles")
    ...
    def _draw_lightning(self):
        ...
        if self.lightning_timer > 60 and random.random() < 0.03:
            try:
                ...
                self.canvas.after(100, remove_flash)
    ...
    def _cleanup_particles(self):
        ...
        try:
            ...
            print("[WeatherAnimation] Particles cleaned up")
        except Exception as e:
...
if __name__ == "__main__":
    def test_fixed_animation():
        """Test the fixed animation system"""
        root = tk.Tk()
        root.title("Fixed Weather Animation Test")
        root.geometry("800x600")
        
        canvas = tk.Canvas(root, bg='skyblue', highlightthickness=0)
        canvas.pack(fill=tk.BOTH, expand=True)
        
        animation = WeatherAnimation(canvas)
        
        # Control panel
        control_frame = tk.Frame(root, bg='white', height=50)
        control_frame.pack(side=tk.BOTTOM, fill=tk.X)
        control_frame.pack_propagate(False)
        
        current_weather = tk.StringVar(value="rain")
        
        weathers = ["rain", "snow", "storm", "cloudy", "sunny", "clear", "mist"]
        
        for weather in weathers:
            btn = tk.Button(
                control_frame,
                text=weather.title(),
                command=lambda w=weather: [
                    animation.set_weather_type(w),
                    current_weather.set(w),
                    root.title(f"Fixed Animation - {w.title()}")
                ],
                width=8
            )
        ...
        tk.Button(
            control_frame,
            text="Start",
            command=lambda: animation.start_animation(current_weather.get()),
            bg='lightgreen',
            width=8
        ...
        # Status display
        status_label = tk.Label(control_frame, text="Ready", bg='white')
        status_label.pack(side=tk.RIGHT, padx=10, pady=5)
        
        def update_status():
            count = animation.get_particle_count()
            running = animation.is_animation_running()
            weather = animation.get_current_weather()
            status_label.config(text=f"{weather}: {count} particles, {'running' if running else 'stopped'}")
        ...
        print("🎬 Fixed animation test started - close window to exit")
        root.mainloop()
