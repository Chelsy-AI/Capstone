...
from .display import show_weather_history


def get_lat_lon(city):
    """
    Get latitude and longitude for a city name using Nominatim geocoder.
...
def fetch_world_history(city):
    """
    Fetch 7-day historical weather data for a city from Open-Meteo archive API.
...
def show_weather_history(text_widget, city="New York"):
    """
    Display 7-day historical temperature data in a Tkinter text widget.

    Args:
        text_widget: Tkinter Text widget where the data will be inserted.
        city (str): City name (default is "New York").
    """
    ...
    for i in range(len(days)):
        ...
        total_temp += avg
        count += 1
        # Fix typo here: print tmin instead of literal string "(tmin)"
